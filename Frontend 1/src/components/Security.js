...
class Security extends Component {
    ...
    componentDidMount() {
        ...
        axios.get('http://127.0.0.1:5003/security', {
            headers: {
    ...
    verifyuser = (e) => {
        e.preventDefault();
        axios.post('http://127.0.0.1:5003/securityPost', { answer: this.state.answer, OriginalAnswer: this.state.OriginalAnswer, email: this.state.email }, {
            headers: {
              Authorization: 'Bearer ' + localStorage.getItem('IdToken')  
            }})
            .then((res) => {
